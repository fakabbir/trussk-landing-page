...
function Code({ children, max = 'max-h-56' }) {
  return (
    <pre
      className={`overflow-auto ${max} rounded-lg border border-white/8 bg-ink/70 p-3 font-mono text-[11px] leading-relaxed whitespace-pre text-muted`}
    >
      {children}
    </pre>
  )
...
function Answer({ text, forbidden }) {
  ...
  return (
    <div
      className="max-h-72 overflow-y-auto text-[13px] leading-relaxed text-muted"
  ...
  return (
    <div className="rounded-xl border border-white/8 bg-surface/40 p-5">
      <div className="flex flex-wrap items-baseline gap-3">
        ...
        <span className="font-mono text-[10.5px] text-muted/70">expected: {q.expect}</span>
      </div>

      <p className="mt-3 border-l-2 border-accent/40 pl-4 font-serif text-[15.5px] leading-relaxed text-[#e8edf4]/90">
        {q.question}
      </p>
      ...
      <div className="mt-5 grid gap-3 lg:grid-cols-3">
        {MODES.map((m) => {
          ...
          return (
            <div key={m} className="rounded-lg border border-white/8 bg-ink/40">
              ...
              <div className="space-y-3 p-4">
                      ...
                      <tbody>
                        {s.retrieved.map((h, i) => (
                          <tr key={i} className="border-b border-white/6 last:border-0">
                            <td className="w-11 py-1 text-muted">{h.sim.toFixed(3)}</td>
                ...
                <dl className="grid grid-cols-2 gap-y-1 border-t border-white/8 pt-3 font-mono text-[10.5px] tabular-nums">
                  {[
                    ...
                    <div key={k} className="col-span-2 flex justify-between gap-3">
                      ...
                      <dd className="text-[#e8edf4]">{v === null ? '—' : v}</dd>
                    </div>
                  ))}
                </dl>
  ...
  return (
    <>
      ...
      <Section label="Results" note="3 trials per question/mode">
        ...
        <div className="mt-6 overflow-x-auto rounded-xl border border-white/8 bg-surface/60">
          <table className="w-full min-w-[620px] border-collapse text-sm">
            <thead>
              <tr className="border-b border-white/10 bg-white/[0.02] text-left font-mono text-[10px] uppercase tracking-[0.14em] text-muted">
                <th className="px-5 py-3 font-medium">Across all 12 runs</th>
                {MODES.map((m) => (
                  <th key={m} className="px-5 py-3 font-medium text-[#e8edf4]">
                    {MODE_META[m].label}
                  </th>
                ))}
              </tr>
            ...
            <tbody className="font-mono text-[12.5px] tabular-nums">
              {[
                ...
                ['hallucinations', (t) => t.halluc, (t) => (t.halluc ? 'text-red-300' : '')],
                ['right answer, no citation', (t) => t.uncited, (t) => (t.uncited ? 'text-warm' : '')],
                ...
                <tr key={label} className="border-b border-white/6 last:border-0">
                  <th scope="row" className="px-5 py-2.5 text-left font-normal text-muted">
                    {label}
                  </th>
                  {MODES.map((m) => (
                    <td key={m} className={`px-5 py-2.5 ${cls?.(data.totals[m]) || 'text-[#e8edf4]'}`}>
                      {get(data.totals[m])}
                    </td>
                  ))}
                </tr>
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          <Card>
            ...
            <table className="mt-3 w-full border-collapse font-mono text-[11.5px] tabular-nums">
              <thead>
                <tr className="border-b border-white/15 text-left text-[10px] uppercase tracking-wider text-muted">
                ...
                {c.sectionsByItem.map((r) => (
                  <tr
                    ...
                    className={`border-b border-white/6 last:border-0 ${
                      r.item === '1A' ? 'text-[#e8edf4]' : 'text-muted'
                    }`}
                  >
          ...
          <Card>
                ...
                return (
                  <div key={label}>
                    <div className="flex justify-between font-mono text-[11px] tabular-nums">
                      ...
                      <span className="text-[#e8edf4]">{secs.toFixed(2)}s</span>
                    </div>
                    <div className="mt-1 h-1 overflow-hidden rounded bg-white/8">
                      <div
            ...
            <p className="mt-4 font-mono text-[10.5px] leading-relaxed text-muted/70">
            ...
            <button
              ...
              className={`rounded-full border px-3.5 py-1.5 text-[12.5px] transition-colors ${
                openId === q.id
                  ...
                  : 'border-white/10 bg-surface/60 text-muted hover:border-accent/30 hover:text-[#e8edf4]'
              }`}
            >
