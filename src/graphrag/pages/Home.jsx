...
function Stat({ value, label }) {
  return (
    <div>
      <div className="font-serif text-3xl tabular-nums tracking-tight text-accent">{value}</div>
...
function ArchColumn({ title, subtitle, rows, accent }) {
  return (
    <Card className="flex flex-col">
      <h3 className="font-mono text-[12px] font-semibold uppercase tracking-wider text-[#e8edf4]">
        {title}
      </h3>
      ...
      <ul className="mt-4 space-y-2 text-[13px] text-muted">
        {rows.map((r) => (
          <li key={r} className="flex gap-2">
            <span className={accent ? 'text-accent' : 'text-muted/50'}>→</span>
            <span>{r}</span>
          </li>
        ))}
      </ul>
    ...
    <>
      ...
      >
        <p>
          ...
          <em className="text-[#e8edf4] not-italic">identity</em> and{' '}
          <em className="text-[#e8edf4] not-italic">provenance</em> are structure you either
      ...
      <Section label="Quick compare" note="one square per trial · rows are questions">
        ...
        <p className="mt-5 text-[13.5px] leading-relaxed text-muted">
          ...
          <Link to="/graphrag/benchmarks" className="text-accent hover:underline">
            Full methodology →
          </Link>
        </p>
      </Section>
      ...
      <Section label="Why similarity search cannot reach the answer" note="measured, not asserted">
        <div className="grid gap-4 md:grid-cols-2">
          <Card>
            <p className="font-serif text-3xl tabular-nums text-accent">0</p>
            ...
            <p className="mt-3 text-[13.5px] leading-relaxed text-muted">
              ...
              <strong className="text-[#e8edf4]">none</strong> of the extracted 10-K sections —
            ...
            <p className="mt-4 font-serif text-3xl tabular-nums text-accent">0</p>
            <p className="mt-1 font-mono text-[11px] uppercase tracking-wider text-muted">
          ...
          <Card>
            ...
            <p className="mt-2 text-[13.5px] leading-relaxed text-muted">
              text-to-SQL matched{' '}
              <code className="font-mono text-[12px] text-warm">owner_name ILIKE '%polk%'</code>{' '}
              and reported all four as one person. The graph keys{' '}
              <code className="font-mono text-[12px] text-accent">:Person</code> by CIK, so
                  ...
                  {c.polk.map((p) => (
                    <tr
                      ...
                      className={`border-b border-white/6 last:border-0 ${
                        p.subject ? 'bg-accent/8 text-[#e8edf4]' : 'text-muted'
                      }`}
                    >
            ...
            <p className="mt-3 font-mono text-[10.5px] leading-relaxed text-muted/70">
            ...
            <dl className="mt-3 grid grid-cols-2 gap-y-1.5 font-mono text-[12px] tabular-nums">
              {[
                ...
                <div key={k} className="col-span-2 flex justify-between gap-4">
                  ...
                  <dd className="text-[#e8edf4]">{fmt(v)}</dd>
                </div>
              ))}
            </dl>
          </Card>
      ...
      <Section>
        <div className="flex flex-col gap-4 rounded-xl border border-accent/20 bg-accent/[0.04] p-7 sm:flex-row sm:items-center sm:justify-between">
          <div>
          ...
          <Link
            ...
            className="shrink-0 rounded-lg bg-accent px-5 py-2.5 text-center text-[14px] font-semibold text-ink transition-opacity hover:opacity-90"
          >
            Open the playground
          </Link>
        </div>
